...
$footer = $basePath+"ClientApp/src/Components/Navigation/Footer.tsx"
(Get-Content -Path $footer) |
    ForEach-Object {$_ -Replace $OldValue, $NewValue} |
        Set-Content -Path $footer

$info = "Updated Footer.tsx with "+ $OldValue +" to "+ $NewValue +"`r`n"
Write-Output $info
...
dotnet publish "../src/Temple.sln" -c Release -r win-x86 --self-contained true --output $outputPath
